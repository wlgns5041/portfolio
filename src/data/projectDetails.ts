...
단순 정적 페이지가 아니라 “프로덕트처럼 운영 가능한 포트폴리오”를 목표로, 컴포넌트 단위 설계/타입 안정성/성능/배포까지 전체 흐름을 직접 구성했습니다.`,
    ...
    techReasons: `• TypeScript
                  포트폴리오는 시간이 지나며 섹션/데이터 구조가 계속 변경되기 때문에, props/데이터 모델의 변경이 화면 전체에 전파될 때 “런타임에서 깨지는 문제”가 발생하기 쉽습니다.
                  ProjectItem, ProjectDetail 등 도메인 타입을 먼저 정의해 데이터 구조를 고정하고, 컴포넌트 간 계약(Props)을 타입으로 관리해 리팩터링 시 안정성을 확보했습니다.
                  (추가로, 모달/프로젝트 상세 같은 복잡한 UI 상태에서도 잘못된 타입 전달을 컴파일 단계에서 차단하는 효과가 있었습니다.)

                  • Tailwind CSS
                  페이지 전체에 공통 UI 패턴(카드/배지/버튼/모달/그라데이션)이 반복되기 때문에, 전통적인 CSS로 관리하면 스타일 스코프 충돌과 유지보수 비용이 빠르게 커집니다.
                  Tailwind 유틸리티 기반으로 디자인 토큰을 일정하게 유지하고, 컴포넌트 단위로 스타일을 “같은 규칙”으로 확장할 수 있어 선택했습니다.
                  또한 빠른 프로토타이핑 → 고정 UI 컴포넌트화 흐름에서 생산성이 높았습니다.

                  • React + Framer Motion
                  섹션 단위로 레이아웃을 분리하고, 모달/탭/스크롤 기반 인터랙션을 상태로 관리하기 위해 React 구조가 적합했습니다.
                  Framer Motion은 모달 오픈/클로즈, 스크롤 기반 애니메이션을 CSS만으로 관리할 때 생기는 복잡도를 줄이고, motion value 기반으로 자연스러운 전환을 만들 수 있어 채택했습니다.

                  • Vercel
                  정적 배포에 최적화되어 있고, PR 단위 Preview 배포/빌드 로그 확인/환경변수 관리가 쉬워 운영 관점에서 가장 비용이 낮아 선택했습니다.`,
